...
$TP=($H=(hostname); Get-ChildItem -Path Cert:LocalMachine\MY | where Subject -Like "CN=${H}*" | Select-Object -Property Thumbprint); winrm create winrm/config/Listener?Address=*+Transport=HTTPS '@{Hostname="p-na26-ica1.extendhealth.com"; CertificateThumbprint="$TP"}'
